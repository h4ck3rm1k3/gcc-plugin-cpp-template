...
void
TC_TRANSLATION_UNIT_DECL::finish_unit (tree t)
{
  ...
  if (ty)
    {
      call_type_ret < CallBack, int >(ty, CallBack::finish_unit_callback);
    }

  t = TREE_CHAIN (t);
  while (t)
    {
      call_type_ret < CallBack, int >(t, CallBack::finish_unit_callback);
      t = TREE_CHAIN (t);
    }
}
