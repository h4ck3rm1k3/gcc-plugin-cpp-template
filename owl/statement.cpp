...
Statement::Statement (const Uri & sub, const Uri & pred, const Uri & obj)
{
...
Statement::Statement (const Uri & sub, const Uri & pred, int obj)
{
  librdf_statement *st;
  st = librdf_new_statement (rdf_world::get_world ());
  ...
  librdf_statement_set_subject (st, node_create_from_uri (sub.get_uri ()));
  librdf_statement_set_predicate (st, node_create_from_uri (pred.get_uri ()));
  librdf_statement_set_object (st,
			       node_create_from_string (std::to_string (obj).
							c_str ()));

  librdf_model_add_statement (rdf_world::get_model (), st);
}

Statement::Statement (const Uri & sub, const Uri & pred, const char *obj)
{
  //uri=librdf_new_uri(rdf_world::get_world(), (const unsigned char*)url);
  librdf_statement *st;
  st = librdf_new_statement (rdf_world::get_world ());
...
Statement::Statement (const Uri & sub, const ConstUri2 & pred, const ConstUri2 & obj)
{
  //uri=librdf_new_uri(rdf_world::get_world(), (const unsigned char*)url);
  librdf_statement *st;
  st = librdf_new_statement (rdf_world::get_world ());
