...
public:
    ...
    rdf_context_imp ():librdf_type ("hashes"),
    librdf_storagename (stream_name), librdf_options ("hash-type='memory'")
  {

    pworld = librdf_new_world ();
    librdf_world_open (pworld);
    pstorage = librdf_new_storage (pworld, librdf_type.c_str (), 0,	//librdf_storagename.c_str(),
...
void
rdf_context::start_unit (const char *amainfilename)
{
  mainfilename = amainfilename;
}

void
rdf_context::finish_unit ()
{
  if (mainfilename)
    {
...
librdf_world *
rdf_context::get_world ()
{
  return pimp->get_world ();
}

librdf_model *
rdf_context::get_model ()
{
  return pimp->get_model ();
}

rdf_context *
  rdf_world::p_rdf_context = 0;
